  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/80 backdrop-blur-md overflow-y-auto">
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-sm font-extrabold text-white">Interactive Concept Prototype</span>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1 p-1 bg-slate-800/80 rounded-xl border border-slate-700/60">
              ...
              >
                ...
                <span>1. Dentist Oversight View</span>
              </button>
              ...
              >
                ...
                <span>2. Patient Phone Experience</span>
              </button>
              ...
              >
                ...
                <span>3. Practice Owner Economics</span>
              </button>
            </div>
        ...
        {/* Mobile Tab bar */}
        <div className="sm:hidden bg-slate-800 px-4 py-2 flex items-center justify-around border-b border-slate-700">
          <button
            onClick={() => setActiveTab('dentist-oversight')}
            className={`text-xs font-bold py-1 px-2 rounded ${
              activeTab === 'dentist-oversight' ? 'bg-indigo-600 text-white' : 'text-slate-400'
            }`}
          >
            Dentist View
          </button>
          <button
            onClick={() => setActiveTab('patient-mobile')}
            className={`text-xs font-bold py-1 px-2 rounded ${
              activeTab === 'patient-mobile' ? 'bg-indigo-600 text-white' : 'text-slate-400'
            }`}
          >
            Patient Phone
          </button>
          <button
            onClick={() => setActiveTab('practice-roi')}
            className={`text-xs font-bold py-1 px-2 rounded ${
              activeTab === 'practice-roi' ? 'bg-indigo-600 text-white' : 'text-slate-400'
            }`}
          >
            Owner ROI
          </button>
            ...
            <div className="space-y-6 max-w-4xl mx-auto">
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="flex items-center gap-3.5">
                  <div className="w-12 h-12 rounded-full bg-indigo-50 border border-indigo-100 flex items-center justify-center text-primary font-black text-sm">
                    PS
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      {patient.insurance} · Last Exam: {patient.lastSeen}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <div className="text-xs text-slate-400 font-bold uppercase tracking-wider">Total Backlog</div>
                    <div className="text-lg font-black text-slate-800">${patient.totalBacklogValue.toLocaleString()}</div>
                  </div>
                  <div className="h-8 w-px bg-slate-200" />
                  <div>
                    {isSignedOff ? (
                      <span className="px-3 py-1.5 rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs font-bold flex items-center gap-1.5 shadow-sm">
                        <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                        <span>Signed-Off by {dentistName}</span>
                      </span>
                    ) : (
                      <span className="px-3 py-1.5 rounded-xl bg-amber-50 text-amber-700 border border-amber-200 text-xs font-bold flex items-center gap-1.5 shadow-sm animate-pulse">
              ...
              <div className="bg-indigo-50/70 rounded-2xl p-4 border border-indigo-100 flex items-start gap-3">
                <Info className="w-5 h-5 text-indigo-600 shrink-0 mt-0.5" />
                <div className="text-xs text-indigo-900 leading-relaxed">
                  <span className="font-bold">AI Clinical Sequencing Logic: </span>
                  ...
                  >
                    <div className="flex items-start justify-between gap-4 border-b border-slate-100 pb-3">
                      ...
                      <div className="text-right">
                        ...
                        <span className="text-sm font-black text-slate-800">${chapter.subtotal}</span>
                        <span className="text-xs text-emerald-600 font-bold ml-1.5">(Est. Gap: ~${chapter.estimatedNet})</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-3.5 space-y-2.5">
                      {chapter.procedures.map((proc, pIdx) => (
                        <div key={pIdx} className="p-3 rounded-xl bg-slate-50 border border-slate-150 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                          <div className="space-y-0.5">
                            <div className="flex items-center gap-2">
                              <span className="text-xs font-bold text-slate-800">{proc.tooth} · {proc.name}</span>
                          ...
                          <div className="text-right shrink-0">
                            <div className="text-xs font-bold text-slate-700">${proc.cost}</div>
                            <div className="text-[10px] text-emerald-600 font-medium">Rebate Quote: ~${proc.estimatedRebate}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        <button className="text-[11px] font-bold text-slate-500 hover:text-indigo-600 transition-colors">
                          Reorder Procedures
                        </button>
                      </div>
                    </div>
              ...
              <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2.5 w-full sm:w-auto">
                  ...
                  <button
                    onClick={() => setActiveTab('patient-mobile')}
                    className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm transition-all"
                  >
          ...
          {activeTab === 'patient-mobile' && (
            <div className="flex flex-col items-center justify-center py-4">
              ...
              <div className="w-full max-w-sm bg-slate-900 rounded-[40px] p-3 shadow-2xl border-4 border-slate-800 relative">
                ...
                {/* Inner Screen */}
                <div className="bg-[#FAF9F6] rounded-[32px] overflow-hidden text-slate-800 p-4 space-y-3.5 text-xs max-h-[640px] overflow-y-auto">
                  {/* Clinic Branding */}
                  <div className="flex items-center justify-between border-b border-slate-200/80 pb-2.5">
                    <div>
                      <h4 className="font-black text-slate-900 text-xs">{clinicName}</h4>
                      <p className="text-[10px] text-slate-400">Dr. Sarah Chen · Clinical Care Plan</p>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-1.5 text-center">
                    <div className="p-2 rounded-xl bg-indigo-600 text-white font-bold text-[10px] shadow-sm">
                  ...
                  <div className="bg-white rounded-2xl p-3.5 border-2 border-indigo-600 shadow-md space-y-2.5 relative">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-150 space-y-1">
                      <div className="flex justify-between text-[11px]">
                        ...
                        <span className="font-bold text-slate-800">$590</span>
                      </div>
                      ...
                      <div className="pt-1 border-t border-slate-200 flex justify-between font-extrabold text-xs text-slate-900">
                        ...
                        <span className="text-indigo-600">~$190</span>
                      </div>
                    ...
                    <button className="w-full py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-md flex items-center justify-center gap-1.5 transition-all">
                  ...
                  <div className="bg-slate-100/90 rounded-2xl p-3 border border-slate-200 space-y-1.5 opacity-80">
                    <div className="flex items-center justify-between">
                  ...
                  <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-2.5">
                    ...
                    <div className="space-y-1 text-xs text-slate-300">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-rose-300">76%</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Patients Converting:</span>
                        <span className="font-bold text-white">24 patients</span>
                      </div>
                    </div>
                    <div className="pt-2 border-t border-white/10 flex justify-between items-center text-sm font-black">
                      ...
                      <span className="text-white">$76,800</span>
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 space-y-2.5 ring-1 ring-emerald-500/20">
                    ...
                    <div className="space-y-1 text-xs text-slate-200">
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-emerald-300">68% (68 patients)</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Phase 2 Recall Conversion ($1,650):</span>
                        <span className="font-bold text-emerald-300">54% (54 patients)</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Phase 3 Routine Retention ($180):</span>
                        <span className="font-bold text-emerald-300">48% (48 patients)</span>
                      </div>
                    </div>
                    <div className="pt-2 border-t border-emerald-500/20 flex justify-between items-center text-sm font-black">
                      <span className="text-emerald-200">Total Practice Production:</span>
                      <span className="text-emerald-400">$137,860 (+79.5%)</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      ...
                      className={`text-sm ${star <= feedbackRating ? 'text-amber-400' : 'text-slate-200'} transition-transform hover:scale-110`}
                    >
                      ★
                    </button>
                  ))}
              ...
              <div className="flex items-center gap-2 w-full md:w-auto">
                <button
                  ...
                  className="w-full md:w-auto px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold shadow-sm transition-all"
                >
