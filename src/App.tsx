...
export default function App() {
  ...
  return (
    <div id="dentai-viewport" className="min-h-screen bg-[#070B11] text-slate-100 selection:bg-cyan-500/30 selection:text-cyan-200">
      {view === 'history' && (
        <HistoryHub
      ...
      <AnimatePresence>
        {showInactivityWarning && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/75 backdrop-blur-md px-4">
            <div className="p-0.5 rounded-2xl bg-gradient-to-b from-amber-500/30 via-[#1E2E40] to-transparent border border-amber-500/30 shadow-2xl max-w-sm w-full mx-auto font-sans">
              <div className="bg-[#0E1622] rounded-[calc(1rem-2px)] p-6 flex flex-col items-center text-center">
                <div className="w-12 h-12 bg-amber-500/15 text-amber-400 border border-amber-500/30 rounded-full flex items-center justify-center mb-4">
                ...
                <p className="text-slate-400 text-xs mb-6 leading-relaxed font-sans">
                  Due to inactivity, you will be logged out automatically in <span className="font-mono font-bold text-amber-400 text-sm">{inactivityCountdown}</span> seconds to protect patient clinical records.
                </p>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="w-full h-11 bg-gradient-to-r from-cyan-400 to-teal-400 hover:from-cyan-300 hover:to-teal-300 text-slate-950 font-black rounded-xl transition-all cursor-pointer text-xs shadow-lg shadow-cyan-950/50 font-sans active:scale-98"
                >
                  Keep Me Logged In
                </button>
              </div>
