...
function finalizeHostedNoteOutput(
  result: { ok: true; output: any; provider?: string; model?: string },
  transcript: any[],
  ...
  output.sovereignty = {
    dataSovereignty:
      (result as any).provider === 'ollama' || (result as any).provider === 'llama-cpp'
        ? 'LOCAL_ON_PREM'
        : (process.env.GCP_PROJECT_ID ? 'AU_SYDNEY' : 'DEVELOPER_API_FALLBACK'),
    jurisdiction: 'APP_8_COMPLIANT',
  ...
  try {
    ...
    if (result.ok === false) {
      ...
      return res.status(500).json({
        error: result.message || 'Failed to process clinical transcript and generate notes.',
        code: 'API_ERROR'
      });
    }

    const output = finalizeHostedNoteOutput(result, transcript, 'consultation-direct');
