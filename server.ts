...
import { createTranscriptValidation, clinicalHorizonFilter, clinicalHorizonFilterStats, transcriptFingerprint } from './src/server/payloadValidation';
...
async function computeFunnelMetrics(): Promise<FunnelMetrics> {
  ...
  // A plan that costs nothing is not an upgrade. 'trial' and 'solo' are both free
  // (PLANS[...].monthlyAudExGst === 0), so counting them here made the conversion
  // figure partly a function of the free tier existing. Note this is deliberately a
  // free-plan blacklist and not a price test: 'enterprise' (Group) is quote-priced
  // at 0 in PLANS, so a `price > 0` filter would silently drop real revenue.
  const FREE_PLAN_IDS: PlanId[] = ['trial', 'solo'];
  const upgraded = subscriptions.filter(
    (s: any) => s.plan && !FREE_PLAN_IDS.includes(s.plan) && s.status === 'active'
  ).length;
...
interface JobRecord {
  ...
  payload: {
    intakeData: any;
    transcript: any[];
    /** Provenance of the generation input, recorded at submission time. */
    generationInput?: {
        ...
        try {
          ...
          const consult: any = {
            ...
            generationProvenance: {
              ...
              thinkingLevel: resolveNoteThinkingLevel(),
              transcriptHash: job.payload?.generationInput?.transcriptHash || null,
              utterancesRaw: job.payload?.generationInput?.utterancesRaw ?? (job.payload?.transcript?.length || 0),
              horizonDropped: job.payload?.generationInput?.horizonDropped ?? 0
            },
            groundingReport,
          ...
          logAudit('note_job_consultation_persisted', job.dentistId, {
            jobId: job.id,
            transcriptHash: job.payload?.generationInput?.transcriptHash || null,
            utterancesRaw: job.payload?.generationInput?.utterancesRaw ?? null,
            horizonDropped: job.payload?.generationInput?.horizonDropped ?? null
          });
...
app.post('/api/notes/jobs', authenticateToken, async (req: any, res: express.Response) => {
  try {
    ...
    const isUuid = (v: unknown): v is string =>
      ...
      // The record keeps the VERBATIM transcript. Only the generation input is
      // horizon-filtered (buildNotePrompt filters again at generation time), so
      // a completed job can never persist a thinned record of the appointment.
      const generationInput = {
        transcriptHash: transcriptFingerprint(transcript),
        utterancesRaw: transcript.length,
        horizonDropped: clinicalHorizonFilterStats(transcript).dropped
      };
      const job: JobRecord = {
        ...
        status: 'queued',
        attempts: 0,
        payload: { intakeData, transcript, generationInput, consentObtained, consentCapturedAt, consentPractitionerId },
        createdAt: new Date().toISOString(),
        nextAttemptAt: null
      };
...
app.post('/api/generate-notes', authenticateToken, async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    // Provenance: tie this generation to the exact verbatim record that produced
    // it — the hash, the raw size, and what the horizon filter trims. A trim is
    // never silent, and a medicolegal review can always find the spoken source.
    ...
    logAudit('note_generation_provenance', (req as any).dentist?.id || 'unknown', generationProvenance);
