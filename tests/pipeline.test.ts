...
describe('Treatment Revenue Moat & ADA Valuation Engine', () => {
  ...
  beforeAll(async () => {
    ...
    const regRes = await request(app)
      ...
      // A non-guessable PIN: registration refuses repeated digits, runs like
      // 1234, and the common values (see isWeakPin in src/lib/authPolicy.ts).
      .send({ name: testName, specialty: 'General Dentistry', pin: '5083' });
